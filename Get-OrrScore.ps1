...
function Get-DisplayRemark {
    ...
    return $Remark -replace '<BIT_ID>', $BitId
}
...
function Write-CheckpointTable {
    ...
    $tableRows = for ($groupIndex = 0; $groupIndex -lt $groups.Count; $groupIndex++) {
        ...
        $checkpointText = ($uniqueCheckpoints | ForEach-Object {
                '{0} ({1}) - {2} [{3}] {4}' -f $_.Checkpoint, $_.CheckpointVersion, $_.Name, $_.Status, (Get-DisplayRemark -Remark $_.NormalizedRemark -BitId $_.BitId)
            }) -join '; '
        [pscustomobject]@{
            Number = $groupIndex + 1
            BitId = $first.BitId
    ...
    foreach ($row in $tableRows) {
        $checkpointLines = @($row.FailedCheckpoints -split '(?<=; )' | ForEach-Object {
                $remaining = $_.Trim()
...
function New-CheckpointHtml {
    ...
    if ($ReportMode -ne 'Table') {
        $sections = for ($groupIndex = 0; $groupIndex -lt $groups.Count; $groupIndex++) {
            ...
            if ($ReportMode -eq 'Detailed') {
                $checkpointHtml = ($group.Group | Sort-Object Checkpoint, CheckpointVersion, Result, Status, NormalizedRemark -Unique | ForEach-Object {
                    ...
                    $fields = @(
                        ...
                        ('<div><b>Lifecycle:</b> <font color="{0}"><i>[ {1} ]</i></font></div>' -f $lifecycleColor, (ConvertTo-HtmlDisplayValue $_.Status)),
            ...
            $checkpointHtml = ($group.Group | Sort-Object Checkpoint, CheckpointVersion, Result, Status, NormalizedRemark -Unique | ForEach-Object {
                    ...
                    '<div class="detail"><a href="{0}" target="_blank">{1} ({2}) - {3}</a> <font color="{4}"><i>[ {5} ]</i></font><div class="remark">{6}</div></div>' -f $_.CheckpointUrl, $id, $version, $name, $lifecycleColor, $_.Status, $remark
                }) -join "`n"
            '<section><h2>{0}. {1} [{2}]</h2><p><strong>Last Evaluation (UTC):</strong> {3} &nbsp; <strong>Score:</strong> {4}</p>{5}</section>' -f ($groupIndex + 1), ([System.Net.WebUtility]::HtmlEncode(([string]$first.Application))), ([System.Net.WebUtility]::HtmlEncode(([string]$first.BitId))), ([System.Net.WebUtility]::HtmlEncode(([string]$first.GeneratedAtUtc))), ([System.Net.WebUtility]::HtmlEncode(([string]$first.Score))), $checkpointHtml
        }
    ...
    $htmlRows = foreach ($groupIndex in 0..($groups.Count - 1)) {
        ...
        $first = $group.Group[0]
        $uniqueCheckpoints = @($group.Group | Sort-Object Checkpoint, CheckpointVersion, Result, Status, NormalizedRemark -Unique)
        $checkpointHtml = ($uniqueCheckpoints | ForEach-Object {
                $id = [System.Net.WebUtility]::HtmlEncode(([string]$_.Checkpoint))
                $name = [System.Net.WebUtility]::HtmlEncode(([string]$_.Name))
                $remark = [System.Net.WebUtility]::HtmlEncode((Get-DisplayRemark -Remark $_.NormalizedRemark -BitId $_.BitId))
                $version = [System.Net.WebUtility]::HtmlEncode(([string]$_.CheckpointVersion))
                ...
                $chipClass = switch -Regex ([string]$_.Status) {
                    ...
                    $lifecycleColor = switch ($chipClass) { 'preview' { '#666666' }; 'exception' { '#c00000' }; 'maintenance' { '#b36b00' }; default { '#4da3ff' } }
                    '<a href="{0}" target="_blank">{1} ({2}) - {3}</a> <font color="{4}"><i>[ {5} ]</i></font> <span class="remark">{6}</span><br>' -f $_.CheckpointUrl, $id, $version, $name, $lifecycleColor, $_.Status, $remark
            }) -join "`n"
        $cells = @(
            ($groupIndex + 1),
            $first.BitId,
        ...
        '<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td></tr>' -f (($cells[0..4] | ForEach-Object { [System.Net.WebUtility]::HtmlEncode(([string]$_) ) }) + $cells[5])
    }
    $htmlRows -join "`n"
}
...
function Export-CheckpointPdf {
    ...
    $content = if ($ReportMode -eq 'Table') {
        '<table><thead><tr><th>No.</th><th>BIT ID</th><th>App Name</th><th>Last Evaluation (UTC)</th><th>Score</th><th>Failed Checkpoints</th></tr></thead><tbody>' + $htmlRows + '</tbody></table>'
    } else { $htmlRows }
    $html = @"
<!doctype html>
<html><head><meta charset="utf-8"><title>ORR Checkpoint Report</title>
<style>body{font-family:Arial,sans-serif;font-size:10px;color:#000}h1{color:#000}table{border-collapse:collapse;width:100%;table-layout:fixed;background:transparent}th,td{border:1px solid #777;padding:6px;vertical-align:top;color:#000;overflow-wrap:anywhere}th{font-weight:bold;text-align:left}a{color:#0563c1;text-decoration:underline}.chip{display:inline-block;border-radius:10px;padding:2px 7px;font-size:9px;font-weight:bold;color:#000}.preview{background:#d9d9d9}.exception{background:#f4aaaa}.live{background:#a8c7e8}.maintenance{background:#f6b33f}.remark{color:#000}@page{size:landscape;margin:12mm}</style>
...
if (-not $PSBoundParameters.ContainsKey('Preview')) {
    $Preview = Read-OptionalBoolean -Prompt 'Show preview checkpoints?' -Default $false
}
if (-not $PSBoundParameters.ContainsKey('Exception')) {
    $Exception = Read-OptionalBoolean -Prompt 'Show exception checkpoints?' -Default $false
}
if (-not $PSBoundParameters.ContainsKey('Maintenance')) {
    $Maintenance = Read-OptionalBoolean -Prompt 'Show maintenance checkpoints?' -Default $false
}
if (-not $PSBoundParameters.ContainsKey('Failing')) {
    $Failing = Read-OptionalBoolean -Prompt 'Show failing checkpoints?' -Default $true
}
...
Write-Field -Label 'Failing' -Value $Failing
...
try {
    ...
    $results = foreach ($latest in $latestEvaluations) {
        ...
        $matchingCheckpoints = @(
            $detail.checkpoints | Where-Object {
                ...
                $matchesLifecycleFilter -and $isFailing -eq $Failing
            }
        )
    ...
    if ($OutputMode -in @('Table', 'Summary')) {
        ...
        if ($OutputMode -eq 'Table') {
            Write-Section -Title "CHECKPOINT TABLE ($($summaryRows.Count) row(s))" -Color $(if ($Failing) { 'Red' } else { 'Cyan' })
        ...
        $resultLabel = if ($Failing) { 'FAILING' } else { 'NON-FAILING' }
        $summaryGroups = @(
            $summaryRows |
